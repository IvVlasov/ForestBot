...
@user_router.message(Violation.s2)
async def descr(message: types.Message, state: FSMContext):
    await state.update_data(descr=message.text)
    text = MSGS['steps']['place']
    btn = await buttons.location_btn()
    await message.answer(text, reply_markup=btn)
    await state.set_state(Violation.s3)


@user_router.message(F.location, Violation.s3)
async def place_location(message: types.Message, state: FSMContext):
    place = 'Отправлена геопозиция. Широта: %s Долгота: %s' % (message.location.latitude, message.location.latitude)
    await place_next(message, state, place)


@user_router.message(F.text, Violation.s3)
async def place_text(message: types.Message, state: FSMContext):
    await place_next(message, state, message.text)


async def place_next(message: types.Message, state: FSMContext, place):
...
@user_router.message(F.contact, Violation.s4)
...
@user_router.message(F.text, Violation.s5)
...
@user_router.message(Violation.s6)
...
async def send_confirm_msg(message: types.Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=btn)
    await state.set_state(Violation.s7)
